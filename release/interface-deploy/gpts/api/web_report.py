...
def _get_dictation_service():
    global dictation_service

    if dictation_service is not None:
        return dictation_service

    try:
        from services.report_dictation_service import ReportDictationService
    except Exception as exc:
        current_app.logger.warning("dictation service unavailable: %s", exc)
        return None
    ...
    try:
        ...
        current_app.logger.warning("dictation service initialization failed: %s", exc)
        return None
...
def transcribe_dictation(session_id: str):
    ...
    service = _get_dictation_service()
    if service is None or not hasattr(service, "transcribe_and_summarize"):
        return jsonify({"success": False, "error": "Dictation service is unavailable in this build"}), 503

    try:
        payload = service.transcribe_and_summarize(
