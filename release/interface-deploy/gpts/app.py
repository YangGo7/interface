...
# Windows environments can inherit incorrect registry MIME mappings for JS/WASM.
# Register the frontend asset types explicitly so module scripts load correctly.
...
ENABLE_OPTIONAL_LLM_ROUTES = _env_flag("ENABLE_OPTIONAL_LLM_ROUTES", True)


def _ensure_case_preview(case_dir: Optional[Path]) -> dict:
    if case_dir is None or not case_dir.exists():
        return {"preview_url": None, "preview_width": None, "preview_height": None}

    preview_path = case_dir / "original.png"
    if preview_path.exists():
        preview_img = cv2.imread(str(preview_path), cv2.IMREAD_UNCHANGED)
        if preview_img is not None:
            height, width = preview_img.shape[:2]
            return {
                "preview_url": f"/temp/{case_dir.name}/{preview_path.name}",
    ...
    if orig_file.suffix.lower() not in (".dcm", ".dicom"):
        return {"preview_url": None, "preview_width": None, "preview_height": None}

    try:
        preview_img = load_image_any(orig_file)
        cv2.imwrite(str(preview_path), preview_img)
        height, width = preview_img.shape[:2]
        app.logger.info("Generated missing preview image for %s", orig_file)
        return {
            "preview_url": f"/temp/{case_dir.name}/{preview_path.name}",
            "preview_width": int(width),
            "preview_height": int(height),
        ...
        app.logger.exception("Failed to generate preview image for %s", orig_file)
        return {"preview_url": None, "preview_width": None, "preview_height": None}


def _case_asset_url(case_dir: Optional[Path], path_value: Optional[str] = None, fallback_name: Optional[str] = None) -> Optional[str]:
    if case_dir is None or not case_dir.exists():
        return None
    ...
    if raw_path:
        names.append(Path(raw_path.replace("\\", "/")).name)
    if fallback_name:
    ...
    for name in names:
        ...
        if candidate.exists():
            return f"/temp/{case_dir.name}/{name}"
...
analysis_service.set_pipeline(_shared_pipeline)
...
def pano_status(job_id):
    ...
    if job["status"] == "done" and job.get("result"):
        ...
        overlay_url = _case_asset_url(case_dir, result.get("overlay_path"), "overlay.png")
        heatmap_url = _case_asset_url(case_dir, result.get("heatmap_overlay_path"), "heatmap_overlay.png")
        resp["result"] = {
...
def detect_status(job_id):
    ...
    preview_meta = _ensure_case_preview(case_dir)

    # Prefer converted PNG preview if exists, else first original.*
    image_url = None
    if case_dir and case_dir.exists():
        if preview_meta["preview_url"]:
            image_url = preview_meta["preview_url"]
        else:
            orig_file = next(case_dir.glob("original.*"), None)
    ...
    if job["status"] == "done" and job.get("result"):
        ...
        image_path_rel = _case_asset_url(case_dir, result.get("image_path"))
        overlay_path_rel = _case_asset_url(case_dir, result.get("overlay_path"), "overlay.png")
        heatmap_path_rel = _case_asset_url(case_dir, result.get("heatmap_overlay_path"), "heatmap_overlay.png")
            
        # Fallback if missing in result (legacy check)
        if not image_path_rel and case_dir and case_dir.exists():
             if preview_meta["preview_url"]:
                 image_path_rel = preview_meta["preview_url"]
             else:
                 orig_file = next(case_dir.glob("original.*"), None)
        ...
        if preview_url:
            ...
            if not preview_url.startswith("/"):
                preview_url = "/" + preview_url
        if not preview_url:
            preview_url = preview_meta["preview_url"]
        ...
        resp["result"] = {
            ...
            "preview_width": result.get("preview_width") or preview_meta["preview_width"],
            "preview_height": result.get("preview_height") or preview_meta["preview_height"],
...
# --- Register Blueprints ---
from api.detect_routes import detect_api
from api.dicom_server_browser import dicom_browser_api
from api.web_report import web_report_api

app.register_blueprint(detect_api, url_prefix='/api')
app.register_blueprint(dicom_browser_api, url_prefix='/api/dicom-server') # Updated prefix to match frontend
app.register_blueprint(web_report_api, url_prefix='/api/web_report')

if ENABLE_OPTIONAL_LLM_ROUTES:
    from api.routes_v2 import api_v2

    app.register_blueprint(api_v2, url_prefix='/api/v2')
else:
    app.logger.info("Optional LLM routes disabled via ENABLE_OPTIONAL_LLM_ROUTES.")
# ---------------------------
