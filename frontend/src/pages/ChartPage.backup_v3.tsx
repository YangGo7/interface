    ...
    if (st === 'angle' && pts.length >= 2) {
      ...
      return (
        <g key={key} {...commonProps}>
          ...
          {!isTemp && <text x={pts[1].x} y={pts[1].y - (20 / scale)} fill={color} fontSize={txtSz} fontWeight="bold" textAnchor="middle" style={{ textShadow: '0px 0px 4px black' }}>{deg.toFixed(1)}°</text>}
        </g>
      );
    }
    if (st === 'roi-free') {
      ...
      <div className="flex flex-1 overflow-hidden bg-[#0a0a0a] relative">
        ...
        <div className="flex-1 overflow-hidden relative flex flex-col">
          <main className="flex-1 overflow-y-auto px-6 py-4 flex flex-col gap-4 w-full max-w-[1600px] mx-auto custom-scrollbar">
            ...
            {/* A. Viewer Section */}
            <section
              className="bg-[#050505] border border-gray-800 rounded-xl flex flex-col relative group w-full overflow-hidden shadow-sm flex-1 h-auto"
              ref={viewerRef}
            ...
            >
              ...
              {showSrc ? (
                <div
                  className="w-full h-full relative cursor-crosshair overflow-hidden flex flex-col items-center justify-end p-0 bg-transparent leading-none"
                ...
                >
                  <div
                    className="relative w-full h-auto mt-auto transition-transform duration-75 ease-out"
                    style={{
                      display: 'grid',
                      placeItems: 'center',
                  ...
                  >
                    ...
                    >
                      ...
                      {shapes.map((s, i) => renderShape(s, false, i))}
                      ...
                      {[...shapes, { points: pendingPoints }].flatMap(s => s.points || []).map((p: any, i) => (
                        <circle key={`pt-${i}`} cx={p.x} cy={p.y} r={6 / scale} fill="#22c55e" stroke="none" />
                      ))}
                      {activeSubTool && tempPoint && <circle cx={tempPoint.x} cy={tempPoint.y} r={6 / scale} fill="#22c55e" opacity={0.5} />}
                    </svg>
                  </div>
                </div>
              ) : (
      ...
      {contextMenu.show && createPortal(
        <>
          ...
          <div
            ...
            style={{ position: 'fixed', top: `${contextMenu.y - 10}px`, left: `${contextMenu.x + 15}px`, zIndex: 9999 }}
          >
