...
class PanoPipeline:
    ...
    def _load_model(self, name: str) -> YOLO:
        ...
        print(
            f"[MODEL LOAD] slot={name} type={info.get('type')} path={path} "
            f"default_imgsz={info.get('default_imgsz')} default_conf={info.get('default_confidence')} "
            f"default_iou={info.get('default_iou')}",
            flush=True,
        )
        model = YOLO(str(path))
    ...
    def _result_summary(self, result) -> Dict[str, Any]:
        boxes_count = int(len(result.boxes)) if getattr(result, "boxes", None) is not None else 0
        masks_count = int(len(result.masks.xy)) if getattr(result, "masks", None) is not None else 0
        ...
        if getattr(result, "boxes", None) is not None and len(result.boxes) > 0:
            try:
                ...
                top_classes = [str(result.names[int(cls_id)]) for cls_id in class_ids[:8]]
            except Exception:
                top_classes = []
        return {
            "boxes": boxes_count,
    ...
    def _run_inference(self, img: np.ndarray) -> Dict[str, Any]:
        ...
        # Default to the original image only. The previous CLAHE/gamma ensemble
        # could select a visually different mask set than the one expected by
        # the original deployment workflow.
        iac_variant_mode = os.environ.get("IAC_VARIANTS", "original").strip().lower()
        if iac_variant_mode == "ensemble":
            ...
            print(f"  [PERF][{self.device.upper()}] iac inference (ensemble:{best_variant}): {results['iac_time']:.3f}s")
        else:
            results['iac_res'] = iac_model.predict(
                source=img,
                imgsz=iac_cfg.get("default_imgsz", 1024),
                conf=0.05,
                iou=iac_cfg.get("default_iou", 0.5),
            ...
            results['iac_time'] = time.perf_counter() - t_iac
            print(f"  [PERF][{self.device.upper()}] iac inference (original): {results['iac_time']:.3f}s")

        for key in ("seg_res", "car_res", "per_res", "iac_res"):
            summary = self._result_summary(results[key])
            print(
                f"[RAW RESULT] {key}: boxes={summary['boxes']} masks={summary['masks']} "
                f"top_classes={summary['top_classes']}",
                flush=True,
            )
    ...
    def run(self, image_path: Path, out_dir: Path) -> Dict[str, Any]:
        ...
        if image_path.suffix.lower() in (".dcm", ".dicom"):
            ...
            preview_rel = to_relative(preview_path)
            preview_height, preview_width = img.shape[:2]
        ...
        teeth_sample = processed_results['final_teeth_objects'][0] if processed_results['final_teeth_objects'] else {}
        print(
            "[OUTPUT SCHEMA] "
            f"teeth_count={len(processed_results['final_teeth_objects'])} "
            f"caries_count={len(processed_results['final_caries_objects'])} "
            f"periapical_count={len(processed_results['periapical_objects'])} "
            f"nerve_contours={len(self._get_contours(processed_results['nerve_mask']))} "
            f"sinus_contours={len(self._get_contours(processed_results['sinus_mask']))} "
            f"teeth_sample_keys={sorted(teeth_sample.keys()) if teeth_sample else []} "
            f"teeth_sample_geometry={teeth_sample.get('geometry') if teeth_sample else None} "
            f"teeth_sample_has_box={bool(teeth_sample.get('box')) if teeth_sample else False} "
            f"teeth_sample_contour_len={len(teeth_sample.get('contour') or []) if teeth_sample else 0}",
            flush=True,
        )
        ...
        return {
            ...
            "preview_width": int(preview_width) if preview_width is not None else None,
            "preview_height": int(preview_height) if preview_height is not None else None,
