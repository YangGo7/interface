...
def _log_runtime_diagnostics() -> None:
    weights_root = Path(app.config["BASE_DIR"]) / "weights"
    app.logger.info(
        "Runtime diagnostics: python=%s version=%s cwd=%s base_dir=%s device=%s torch=%s cuda=%s cv2=%s ultralytics=%s",
        sys.executable,
        sys.version.replace("\n", " "),
        Path.cwd(),
    ...
    for model_name, model_info in (app.config.get("PANO_MODELS", {}) or {}).items():
        rel_path = Path(model_info.get("path", ""))
        resolved_path = rel_path if rel_path.is_absolute() else weights_root / rel_path
        exists = resolved_path.exists()
        app.logger.info(
            "Model diagnostics: slot=%s type=%s path=%s exists=%s size=%s",
            model_name,
            model_info.get("type"),
            resolved_path,
            exists,
            resolved_path.stat().st_size if exists else None,
        )
