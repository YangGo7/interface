...
class ReportGeneratorV3:
    def __init__(self, output_dir=None):
        ...
        self.odonto_dir = BASE_DIR / "imgs" / "Odonto"
        self.warning_icon_path = BASE_DIR / "imgs" / "!" / "icon_red.png"
        self.info_icon_path = BASE_DIR / "imgs" / "!" / "icon_black.png"
        self.logo_path = BASE_DIR / "imgs" / "logo" / "Cyberme_logo.png"
    ...
    def _crop_and_overlay(self, orig_img, box, findings, bl_viz=None, pbl_mm=0, implant_guide=None, nerve_contours=None, sinus_contours=None, safety_guide=None, tooth_contour=None, debug_text=None, nerve_dist_line=None, nerve_dist_mm=0, centerline=False, centerline_axis=None, tooth_label=None, neighbor_contours=None, gap_line_coords=None):
        ...
        for f in findings:
            ...
            thickness = 1 if is_caries else 2
            alpha = 0.5 if is_caries else 1.0 # 0.5 transparency for caries lines so tissue is visible
            
            overlay = crop.copy()
            
            # Helper to draw dashed lines on overlay
            def draw_dashed_polygon(img, pts, col, thk, dash_len=5):
                for i in range(len(pts)):
                    ...
                    while curr_dist < dist:
                        start_x = int(p1[0] + dx * curr_dist)
                        start_y = int(p1[1] + dy * curr_dist)
                        end_x = int(p1[0] + dx * min(curr_dist + dash_len, dist))
                        end_y = int(p1[1] + dy * min(curr_dist + dash_len, dist))
                        cv2.line(img, (start_x, start_y), (end_x, end_y), col, thk)
                        curr_dist += dash_len * 2
            
            if f.get('contour'):
                # Shift contour points to crop coords
                cnt = np.array(f['contour'], dtype=np.int32) - [x1, y1]
                draw_dashed_polygon(overlay, cnt, color, thickness)
            else:
                # If no contour, use box corners
                pts = [(cx1, cy1), (cx2, cy1), (cx2, cy2), (cx1, cy2)]
                draw_dashed_polygon(overlay, pts, color, thickness)
                
            if alpha < 1.0:
                cv2.addWeighted(overlay, alpha, crop, 1 - alpha, 0, crop)
            else:
                crop = overlay
    ...
    def generate(self, user_name, image_path, analysis_result, overlay_path=None, bl_viz_path=None, output_dir=None):
        ...
        """
        # Load Images
        orig_img = load_image_any(Path(image_path))
        ...
        bl_viz_img = None
    def _generate_html(self, user_name, date_str, summary_data, findings, main_img, crop_source_img, bl_viz_img=None, missing_teeth=None, nerve_contours=None, sinus_contours=None, ai_commentary=None, llm_analysis=None, detailed_findings=None, per_tooth_analysis=None, report_note=None, attached_captures=None):
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .legend-bar.triage-1 { background: #dc2626; } /* Red */
        .legend-bar.triage-2 { background: #eab308; } /* Yellow */
        .legend-bar.triage-3 { background: #16a34a; } /* Green */
        .legend-bar.implant { background: transparent; border-top: 4px solid #2563eb; } /* Blue */
        .legend-bar.missing { background: transparent; border-top: 4px dashed #9ca3af; } /* Gray */
        ...
        .odonto-tooth:hover { transform: translateY(-1px); }
        .odonto-tooth img { 
            width: 41px; 
            height: 88px; 
            object-fit: contain;
            filter: brightness(0) invert(1) drop-shadow(0 0 0.55px rgba(255,255,255,1)) drop-shadow(0 0 0.55px rgba(255,255,255,1));
            opacity: 0.98;
        }
        .odonto-tooth.flip img { transform: scaleX(-1); }
        
        /* Triage 3 (Monitor) - Green background */
        .odonto-tooth.triage-3 { background: rgba(34, 197, 94, 0.15); border: 2px solid #22c55e; }
        .odonto-tooth.triage-3 + .odonto-label, .odonto-cell .odonto-label.triage-3 { color: #4ADE80; }
        
        /* Triage 2 (Treatment) - Yellow background */
        .odonto-tooth.triage-2 { background: rgba(234, 179, 8, 0.2); border: 2px solid #eab308; }
        .odonto-tooth.triage-2 + .odonto-label, .odonto-cell .odonto-label.triage-2 { color: #FACC15; }

        /* Triage 1 (Urgent/Extraction) - Red background */
        .odonto-tooth.triage-1 { background: rgba(220, 38, 38, 0.2); border: 2px solid #dc2626; }
        ...
        .odonto-tooth.missing img { opacity: 0.9; filter: brightness(0) saturate(0); }
        .odonto-tooth.missing + .odonto-label, .odonto-cell .odonto-label.missing { color: #CBD5E1; }
        
        /* Implant - blue background */
        .odonto-tooth.implant { background: rgba(37, 99, 235, 0.15); border: 2px solid #2563eb; }
        .odonto-tooth.implant + .odonto-label, .odonto-cell .odonto-label.implant { color: #60A5FA; }
        ...
        .odonto-quadrant { display: flex; gap: 5px; }
        .odonto-quadrant.flip { direction: ltr; }
        .odonto-separator { width: 5px; }
        ...
        /* Keep missing/implant styles for fallback/safety */
        ...
        /* Triage Color Codes */
        .tooth-card.triage-1 { border-left: 5px solid #dc2626; }
        .tooth-card.triage-1 .finding-tag { color: #dc2626; }
        .tooth-card.triage-2 { border-left: 5px solid #eab308; }
        .tooth-card.triage-2 .finding-tag { color: #eab308; }
        .tooth-card.triage-3 { border-left: 5px solid #16a34a; }
        .tooth-card.triage-3 .finding-tag { color: #16a34a; }
        .crop-img { 
            width: 192px; 
            height: 256px; 
            object-fit: contain; 
        ...
        .tooth-details h3 { font-size: 1.75rem; margin-bottom: 12px; text-align: left; }
        .finding-tag { 
            display: inline-flex; 
            align-items: center; 
            gap: 2px; 
        ...
        .description { color: #6b7280; font-size: 0.9rem; margin-bottom: 24px; line-height: 1.6; text-align: left; }
        .treatments h4 { font-size: 1.25rem; font-weight: 700; margin-bottom: 12px; text-align: left; }
        ...
        /* [USER REQUEST] Unified Triage Badge Colors */
        .badge.triage-1 { background:#dc2626; color:#fff; } /* Red - Triage 1 */
        .badge.triage-2 { background:#eab308; color:#fff; } /* Yellow - Triage 2 */
        .badge.triage-3 { background:#16a34a; color:#fff; } /* Green - Triage 3 */
        
        .badge.pbl-1, .badge.pbl-2 { background:#16a34a; color:#fff; } /* Green - Monitor */
        .badge.pbl-3 { background:#eab308; color:#fff; } /* Yellow - Treatment */
        .badge.pbl-4 { background:#dc2626; color:#fff; } /* Red - Urgent */
        
        .badge.perio { background:#eab308; color:#fff; } /* Yellow */
        .badge.caries { background:#eab308; color:#fff; } /* Yellow */
        .badge.overlap { background:#dc2626; color:#fff; } /* Red - Critical */
        .badge.treatment { background:#eab308; color:#fff; } /* Yellow */
        .badge.info { background:#16a34a; color:#fff; } /* Green */
        .badge.implant { background:#2563eb; color:#fff; }
        .footer { 
            display: flex; 
        ...
        if report_note:
            ...
            <div style="margin: 24px 0 32px; padding: 20px 24px; border-radius: 16px; background: #f8fafc; border: 1px solid #dbeafe;">
                <div style="font-size: 0.8rem; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: #0f766e; margin-bottom: 10px;">Report Reference</div>
                <div style="font-size: 1rem; line-height: 1.7; color: #334155; white-space: pre-wrap;">{html_lib.escape(str(report_note or ''))}</div>
            </div>
            """
        ...
        if attached_captures:
            capture_cards_html = "".join(
                f"""
                <div style="padding:12px; border-radius:14px; background:#ffffff; border:1px solid #dbeafe;">
                    <img src="{html_lib.escape(str(capture.get('dataUrl') or ''))}" alt="{html_lib.escape(str(capture.get('label') or 'Capture'))}" style="display:block; width:100%; max-height:180px; object-fit:contain; border-radius:10px; background:#0f172a;" />
                    <div style="margin-top:10px; font-size:0.9rem; color:#334155; font-weight:600;">{html_lib.escape(str(capture.get('createdAt') or capture.get('label') or 'Capture'))}</div>
                    <div style="margin-top:4px; font-size:0.8rem; color:#64748b;">{html_lib.escape(str(capture.get('size') or ''))}</div>
                </div>
                """
                for capture in attached_captures
                if capture.get('dataUrl')
            )
            if capture_cards_html:
                attached_captures_html = f"""
                <div class="divider"></div>
                <div style="margin: 24px 0 32px; padding: 20px 24px; border-radius: 16px; background: #f8fafc; border: 1px solid #dbeafe;">
                    <div style="font-size: 0.8rem; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: #0f766e; margin-bottom: 14px;">Attached Captures</div>
...
{style}
        ...
        <body>
            <div class="header">
                <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                    <img src="data:image/png;base64,{logo_b64}" style="height:120px; margin-top:-15px; margin-bottom:10px; display: block;">
            ...
            <div class="legend">
                <div class="legend-item"><div class="legend-bar triage-3"></div><span style="color:#16a34a">Routine Checkup (Triage 3)</span></div>
                <div class="legend-item"><div class="legend-bar triage-2"></div><span style="color:#eab308">Treatment Required (Triage 2)</span></div>
                <div class="legend-item"><div class="legend-bar triage-1"></div><span style="color:#dc2626">Urgent Priority (Triage 1)</span></div>
                <div class="legend-item"><div class="legend-bar implant"></div><span style="color:#2563eb">Implant</span></div>
                <div class="legend-item"><div class="legend-bar missing"></div><span style="color:#9ca3af">Missing Tooth</span></div>
            </div>
        ...
        if missing_teeth:
            ...
            for group_entry in missing_groups:
                ...
                missing_note_html = (
                    f"""
                    <div style='margin-top:12px; padding:12px 14px; background:#eff6ff; border:1px solid #bfdbfe; border-radius:10px;'>
                        <div style='font-size:0.74rem; font-weight:700; letter-spacing:0.08em; text-transform:uppercase; color:#1d4ed8; margin-bottom:6px;'>Note</div>
                        <div style='font-size:0.95rem; color:#1e293b; white-space:pre-wrap;'>{missing_note}</div>
                    </div>
                    """
                    if missing_note
                    else ""
        ...
        for item in deduped_findings:
             ...
             if is_problem:
                 ...
                 tooth_note_html = (
                     ...
                     <div style='margin-top:12px; padding:12px 14px; background:#eff6ff; border:1px solid #bfdbfe; border-radius:10px;'>
                         <div style='font-size:0.74rem; font-weight:700; letter-spacing:0.08em; text-transform:uppercase; color:#1d4ed8; margin-bottom:6px;'>Note</div>
                         <div style='font-size:0.95rem; color:#1e293b; white-space:pre-wrap;'>{html_lib.escape(tooth_note)}</div>
                     </div>
                     """
                     if tooth_note
                     else ""
                 ...
                 <div class="tooth-card {card_class}" id="tooth-{tooth_label}">
                     ...
                     <div class="tooth-details">
                         ...
                         <br />
                         <div style="margin-bottom:10px; font-size:1rem; font-weight:700; color:#f8fafc;">Analysis:</div>
                         <p>{' '.join(desc).replace('<strong>Analysis:</strong><br>', '', 1)}</p>
                         {tooth_note_html}
                     </div>
                 </div>
                 """
    ...
    def generate(self, user_name, image_path, analysis_result, overlay_path=None, bl_viz_path=None, output_dir=None, ai_commentary=None, llm_analysis=None, detailed_findings=None, per_tooth_analysis=None):
        """
        ...
        main_img = load_image_any(Path(image_path))
        ...
        if overlay_path and overlay_path.exists():
            ...
            if overlay_img is not None:
                # main_img = overlay_img  # [MODIFIED] Keep main image clean per user request
                crop_source_img = main_img.copy() # [FIX] Use clean original image, target contours drawn individually
