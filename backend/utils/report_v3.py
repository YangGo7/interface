...
class ReportGeneratorV2:
    ...
    # === Simplified UI-spec HTML (v3) with disclaimer, overrides previous definitions ===
    def _generate_html(self, user_name, date_str, summary_data, findings, main_img, crop_source_img, bl_viz_img=None, missing_teeth=None, nerve_contours=None, sinus_contours=None):
        if missing_teeth is None:
            missing_teeth = []

        main_b64 = self._encode_image(main_img)

        def as_set(key):
            if isinstance(summary_data, dict) and summary_data.get(key):
                return set(str(x) for x in summary_data[key])
            return set()

        perio_highlight = as_set("perio_highlight")
        extraction_set = as_set("extraction_candidates")
        implant_site_set = as_set("implant_site_candidates")
        implant_teeth = as_set("implant_teeth")
        treatment_teeth = as_set("treatment_teeth")

        def tooth_status(t: int):
            tk = str(t)
            if t in missing_teeth:
                return "missing"
            if tk in implant_teeth:
                return "implant"
            if tk in treatment_teeth or tk in perio_highlight:
                return "treatment"
            return "healthy"

        fills = {
            "healthy": ("#D1FAE5", "#34D399"),
            "treatment": ("#FEE2E2", "#EF4444"),
            "implant": ("#DBEAFE", "#3B82F6"),
            "missing": ("#F1F5F9", "#94A3B8"),
            "extraction": ("#FFEDD5", "#F97316"),
        }

        def odonto_tile(t: int, status: str, ring: bool, label_top: bool):
            jaw = "up" if 11 <= t <= 28 else "down"
            icon_b64 = self._load_odonto_icon(jaw, t % 10)
            fill, border = fills.get(status, fills["healthy"])
            dashed = status == "missing"
            ring_style = "box-shadow: 0 0 0 2px #DC2626 inset;" if ring else ""
            label = f'<div class="tooth-label" style="color:{border}">{t}</div>'
            top = label if label_top else ""
            bottom = "" if label_top else label
            icon_html = f'<img src="data:image/png;base64,{icon_b64}" class="tooth-icon"/>' if icon_b64 else '<div class="tooth-placeholder"></div>'
            return f"""
              <div class="tooth-cell">
                {top}
                <div class="tooth-box" style="background:{fill};border-color:{border};border-style:{'dashed' if dashed else 'solid'};{ring_style}">
                  {icon_html}
                </div>
                {bottom}
              </div>
        ...
        def build_rows(highlight=None, extraction=None, implant_site=None):
            highlight = highlight or set()
            extraction = extraction or set()
            implant_site = implant_site or set()
            rows = []
            for row, top in [([18,17,16,15,14,13,12,11,21,22,23,24,25,26,27,28], True),
                             ([48,47,46,45,44,43,42,41,31,32,33,34,35,36,37,38], False)]:
                cells = []
                for t in row:
                    base_status = tooth_status(t)
                    if str(t) in extraction:
                        base_status = "extraction"
                    ring_flag = str(t) in highlight or str(t) in implant_site
                    cells.append(odonto_tile(t, base_status, ring_flag, label_top=top))
                rows.append(f"<div class='tooth-row'>{''.join(cells)}</div>")
            return "".join(rows)

        def panel_block(title, highlight=None, extraction=None, implant_site=None, extra_legend=False, extra_body=""):
            rows_html = build_rows(highlight, extraction, implant_site)
            legend_extra = ""
            if extra_legend:
                legend_extra = """
                  <span class="chip extraction">Extraction</span>
                  <span class="chip implantsite">Implant Site</span>
                """
            return f"""
              <div class="panel">
                <div class="panel-header"><div class="panel-title">{title}</div></div>
                <div class="legend">
                  <span class="chip healthy">Healthy</span>
                  <span class="chip treatment">Requires treatment</span>
                  <span class="chip implant">Implant</span>
                  <span class="chip missing">Missing</span>
                  {legend_extra}
                </div>
                <div class="odontogram">{rows_html}</div>
                {extra_body}
              </div>
            """

        overview_html = panel_block("Overview")
        perio_html = panel_block("Periodontitis Staging", highlight=perio_highlight)
        implant_extra = """
          <div class="subsection">
            <div class="subheader">Candidates</div>
            <div class="mini-list">Extraction: {ex_list}</div>
            <div class="mini-list">Implant site: {im_list}</div>
          </div>
        """.format(
            ex_list=", ".join(sorted(extraction_set)) if extraction_set else "—",
            im_list=", ".join(sorted(implant_site_set)) if implant_site_set else "—"
        )
        implant_html = panel_block("Implant Treatment Option",
                                   extraction=extraction_set,
                                   implant_site=implant_site_set,
                                   extra_legend=True,
                                   extra_body=implant_extra)

        # Legal-safe notice replaces detailed diagnosis
        findings_html = """
        <div class="notice">
          <div class="notice-title">Important</div>
          <ul>
            <li>AI-generated screening preview only — not a medical diagnosis.</li>
            <li>No probabilities or certainty implied; clinical judgment by a licensed dentist is required.</li>
            <li>Use this report for reference; confirm with full clinical and radiographic evaluation.</li>
          </ul>
        </div>
        """
...
<head>
  ...
  <style>
    ...
    .chip.healthy {{ background:#D1FAE5; border-color:#34D399; }}
    .chip.treatment {{ background:#FEE2E2; border-color:#EF4444; }}
    .chip.implant {{ background:#DBEAFE; border-color:#3B82F6; }}
    .chip.missing {{ background:#F1F5F9; border-color:#94A3B8; }}
    .chip.extraction {{ background:#FFEDD5; border-color:#F97316; }}
    .chip.implantsite {{ border-color:#DC2626; color:#DC2626; background:#FFF; }}
    .odontogram {{
      display: grid;
      gap: 8px;
    }}
...
<body>
  <div class="page">
    <div class="hero">
      {"<img src=\"data:image/jpeg;base64," + main_b64 + "\"/>" if main_b64 else ""}
    </div>
    <div class="tabs">
      <div class="tab active">Overview</div>
      <div class="tab">Periodontitis</div>
      <div class="tab">Implant Option</div>
    </div>
    <div class="panels">
      {overview_html}
      {perio_html}
    ...
    def _crop_and_overlay(self, orig_img, box, findings, bl_viz=None, pbl_mm=0, implant_guide=None, nerve_contours=None, sinus_contours=None, safety_guide=None, tooth_contour=None, debug_text=None, nerve_dist_line=None, nerve_dist_mm=0, centerline=False, centerline_axis=None):
        ...
        y1 = max(0, expansion_y1 - margin_y)
        x2 = min(w, x2 + margin_x)
        y2 = min(h, expansion_y2 + margin_y)
...
{style}
        ...
        <body>
            ...
            <div class="summary-box">
                <h2 class="summary-title">Clinical Summary (진단 요약)</h2>
        ...
        def build_odonto_row(jaw, quadrants):
            """jaw='up'/'down', quadrants=[(q_prefix, positions, reverse, flip)]"""
            row_html = '<div class="odonto-row">'
            for qi, (q_prefix, positions, reverse, flip) in enumerate(quadrants):
                row_html += '<div class="odonto-quadrant">'
                pos_list = list(reversed(positions)) if reverse else positions
                for pos in pos_list:
                    fdi = f"{q_prefix}{pos}"
                    status = tooth_status.get(fdi, '')
                    flip_class = 'flip' if flip else ''
                    icon_b64 = self._load_odonto_icon(jaw, pos)
                    if icon_b64:
                        row_html += f'''<div class="odonto-tooth {status} {flip_class}" onclick="document.getElementById('tooth-{fdi}')?.scrollIntoView({{behavior:'smooth',block:'center'}})">
                            <img src="data:image/png;base64,{icon_b64}" alt="Tooth {fdi}">
                            <span class="odonto-label">{fdi}</span>
                        </div>'''
                row_html += '</div>'
                if qi == 0:
                    row_html += '<div class="odonto-separator"></div>'
            row_html += '</div>'
            return row_html
        
        # Upper jaw: Q1 (18-11) | Q2 (21-28) - Q2 needs flip
        upper_row = build_odonto_row('up', [
            ('1', [8,7,6,5,4,3,2,1], False, False),  # Q1: 18,17,16...11 (no flip)
            ('2', [1,2,3,4,5,6,7,8], False, True)    # Q2: 21,22,23...28 (flip)
        ])
        
        # Lower jaw: Q4 (48-41) | Q3 (31-38) - Q3 needs flip
        lower_row = build_odonto_row('down', [
            ('4', [8,7,6,5,4,3,2,1], False, False),  # Q4: 48,47,46...41 (no flip)
            ('3', [1,2,3,4,5,6,7,8], False, True)    # Q3: 31,32,33...38 (flip)
        ])
        
        odontogram_html = f'''
        <div class="odontogram">
            <h3>Dental Chart (Click to navigate)</h3>
            {upper_row}
                ...
                <div class="tooth-card missing" id="tooth-{lbl}">
                    ...
                    <div class="tooth-details">
                        ...
                        <p style="color:#374151; font-size:0.95rem; margin-bottom:8px;">
                            <strong>Why classified as Missing:</strong> {mt.get('missing_reason', 'No tooth structure detected in the expected position; adjacent teeth present with edentulous gap.')}
                        </p>
                        <p class="description">
                            <strong>Assessment:</strong> Tooth is clinically absent.<br>
                            <strong>Implication:</strong> Potential for alveolar bone resorption and drift of adjacent teeth.<br>
                            <strong>Plan:</strong> Evaluate edentulous ridge for Implant/Bridge.
                        </p>
                        {f'<p style="color:#2563eb; font-size:0.95rem; margin-bottom:8px;"><strong>Implant note:</strong> {implant_note}</p>' if implant_note else ''}
                        <div class="treatments">
                             <h4>Clinical Options</h4>
                             <ol>
                                 <li>1. Implant Placement (Gold Standard)</li>
                                 <li>2. Fixed Partial Denture (Bridge)</li>
                                 <li>3. Removable Prosthesis</li>
                             </ol>
                            {f'<p style="margin-top:12px;color:#2563eb;font-weight:600;">?諭?Available Bone Height: {mt["implant_guide"]["dist_mm"]:.1f} mm</p>' if isinstance(mt, dict) and mt.get('implant_guide') else ''}
                        </div>
                    </div>
                </div>
                """
        ...
        for item in findings:
             ...
             hopeless = bl_level >= 4 or bl_pct >= 60 or pbl_mm >= 6
             severe_bl = bl_level >= 3 or bl_pct >= 40 or pbl_mm >= 4
             ...
             # Formulate Narrative
             if is_problem:
                 # Structure: Evidence -> Conclusion -> Action
                 if not evidence:
    ...
    def generate(self, user_name, image_path, analysis_result, overlay_path=None, bl_viz_path=None, output_dir=None):
        """
        Main entry point.
        user_name: e.g. "Steve_Jobs"
        image_path: Path to original image
        analysis_result: Structured result from inference (teeth objects, diseases, etc)
        """
        # Load Images
        orig_img = cv2.imread(str(image_path))
        if orig_img is None: raise ValueError("Image not found")
        ...
        date_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        teeth_data = analysis_result.get("teeth", [])
        ...
        main_img = overlay_img if overlay_img is not None else orig_img
        ...
        html_content = self._generate_html(
            ...
            teeth_data, 
            main_img, 
            orig_img, # [FIX] Use Clean Original for crops (draw overlays dynamically)
            bl_viz_img, 
        ...
        try:
            ...
            with open(pdf_path, "wb") as pdf_file:
                ...
                if pisa_status.err:
                    print(f"  [REPORT] PDF generation error: {pisa_status.err}")
                    pdf_filename = None
                else:
                    print(f"  [REPORT] PDF generated: {pdf_path}")
        except ImportError as e:
            print(f"  [REPORT] PDF skipped - xhtml2pdf not installed: {e}")
            pdf_filename = None
        except Exception as e:
