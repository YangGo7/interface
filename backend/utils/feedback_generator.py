...
class FeedbackGenerator:
    ...
    def _get_explanation(self, issues):
        ...
        for issue in issues:
            if "Caries" in issue:
                explanations.append("세균에 의해 발생한 충치로 보입니다. 방치 시 신경까지 진행될 수 있습니다.")
            elif "Periapical" in issue:
                explanations.append("치아 뿌리 끝에 염증(치근단 병소)이 의심됩니다. 신경 치료가 필요할 수 있습니다.")
            elif "Bone Loss" in issue:
                explanations.append("치조골 소실이 관찰됩니다. 잇몸 질환(치주염)의 징후입니다.")
            elif "Nerve" in issue:
                explanations.append("🚨 **발치 시 주의 (Extraction Caution)**: 치아 뿌리가 하악 신경관과 겹쳐있습니다. 발치 시 신경 손상 위험이 있으므로 주의가 필요합니다.")
            elif "Sinus" in issue:
                explanations.append("🚨 **발치 시 주의 (Extraction Caution)**: 치아 뿌리가 상악동과 겹쳐있습니다. 발치 시 상악동 천공 위험이 있으므로 주의가 필요합니다.")
        return " ".join(explanations)
    
    def _get_recommendation(self, issues):
        """Returns recommendation text."""
        if any("Caries" in i for i in issues):
            return "레진 또는 인레이 치료 권장"
        elif any("Periapical" in i for i in issues):
            return "신경 치료(근관 치료) 권장"
        elif any("Bone Loss" in i for i in issues):
            return "치주과 상담 권장"
        elif any("Nerve" in i or "Sinus" in i for i in issues):
            return "CT 촬영 및 구강외과 전문의 상담 필수 (발치 주의)"
        return "정기 검진 권장"
